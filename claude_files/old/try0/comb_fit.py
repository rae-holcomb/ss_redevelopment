...
def guess_pairwise_histogram(
    ...
    for P_cand in candidate_periods:
        ...
        # Primary sort key: coverage (fixes the harmonic-ambiguity failure
        # mode, where P, 2P, 3P... all look like local maxima in the
        # spacing histogram, but only the true fundamental explains nearly
        # all of the peaks that were actually found). comb_score is only a
        # tiebreaker among candidates with essentially equal coverage.
        ranked.append((coverage, score, P_cand, t0_cand))
...
def guess_lombscargle(
    ...
    for P_cand in candidate_periods:
        ...
        score = comb_score(acf_lags, acf, P_cand, t0_cand)
        ranked.append((score, P_cand, t0_cand))
