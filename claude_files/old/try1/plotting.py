...
from comb_fit import InitialGuess, CombFitResult, EnsembleResult
...
# --------------------------------------------------------------------------
# Candidate-ranking landscape (find_best_rotation_period)
# --------------------------------------------------------------------------

def plot_candidate_ranking(
    result: EnsembleResult,
    ax: Optional[plt.Axes] = None,
    title: str = "Candidate period ranking (find_best_rotation_period)",
    log_x: bool = True,
    top_n_labeled: int = 3,
):
    """Plot every candidate period tested by find_best_rotation_period
    against its joint-fit reduced chi-squared (lower is better), colored by
    whether it passed the positive-heights gate and had enough peaks
    retained. The winning candidate is highlighted.

    This is mainly a sanity-check / ambiguity-detection tool: if a
    close runner-up has a similar redchi to the winner, that's worth a
    second look (e.g. a genuine harmonic degeneracy) rather than trusting
    the top pick blindly.
    """
    if ax is None:
    ...
    def _frac_positive(c):
        heights = [p["height"] for p in c.fit.per_peak.values()]
        return np.mean([h > 0 for h in heights]) if heights else 0.0

    heights_ok = np.array([_frac_positive(c) >= 0.8 for c in result.candidates])
    enough_peaks = np.array([c.fit.n_peaks_used >= 4 for c in result.candidates])
    passed = heights_ok & enough_peaks
    ...
    ax.scatter([best.P], [best.redchi if np.isfinite(best.redchi) else np.nan],
               color="firebrick", s=90, marker="*", zorder=5, label="winner")

    # label the top few candidates by rank for quick ambiguity-spotting
    for c in result.candidates[:top_n_labeled]:
